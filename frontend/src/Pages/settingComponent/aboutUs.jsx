...
export default function AboutUs() {
    ...
    return (
        <div className="relative animate-in fade-in duration-1000 -m-8 p-8 min-h-screen">
            <style>
                {`
                    ...
                    body.about-us-mode main {
                        ...
                        border-color: rgba(34,197,94,0.05) !important;
                    }
                    body.about-us-mode {
                        background-color: #0a0a0a !important; /* dark theme */
                        overflow-x: hidden;
                    }
                    ...
                    @keyframes glitch {
                        0% { text-shadow: 2px 0 #00ff9d, -2px 0 #00b36b; transform: translate(0); }
                        20% { text-shadow: -2px 0 #00ff9d, 2px 0 #00b36b; transform: translate(-2px, 2px); }
                        40% { text-shadow: 2px 2px #00ff9d, -2px -2px #00b36b; transform: translate(2px, -2px); }
                        60% { text-shadow: -2px -2px #00ff9d, 2px 2px #00b36b; transform: translate(-2px, -2px); }
                        80% { text-shadow: 2px -2px #00ff9d, -2px 2px #00b36b; transform: translate(2px, 2px); }
                        100% { text-shadow: 2px 0 #00ff9d, -2px 0 #00b36b; transform: translate(0); }
                    }
                    .animate-glitch {
            ...
            {/* Background */}
            {createPortal(
                <div className="fixed inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-gray-900/50 via-neutral-950/70 to-black -z-10 animate-in fade-in duration-1000" />,
                document.body
            )}

            {/* Header */}
            <div className="mb-24 border-l-4 border-green-400/60 rounded-l-xl pl-10 relative z-10">
                <h1 className="text-8xl font-black bg-gradient-to-r from-green-300 via-emerald-400 to-green-500 bg-clip-text text-transparent tracking-tighter leading-none mb-4 uppercase">BOOK2HAND</h1>
                <p className="text-2xl text-green-300/80 font-bold uppercase tracking-[0.4em]">5 developers of book2hand</p>
            </div>

            {/* Grid Cards */}
            <div className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-5 gap-4 rounded-3xl shadow-[0_0_100px_rgba(34,197,94,0.15)] backdrop-blur-sm bg-green-950/40 relative z-10">
                {teamMembers.map((member, index) => (
                    <div
                        ...
                        className={`group flex flex-col bg-green-950/60 backdrop-blur-md h-[700px] transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] hover:z-20 hover:scale-[1.05] hover:shadow-[0_0_80px_rgba(34,197,94,0.3)] rounded-2xl cursor-pointer select-none`}
                    >
                        {/* Image Area */}
            ...
            {/* Footer */}
            <div className="mt-32 border-t-8 border-green-500/20 pt-16 flex flex-col md:flex-row justify-between items-start md:items-end gap-12 pb-24 relative z-10">
                <div>
                    <p className="text-6xl font-black text-green-50/90 tracking-tighter uppercase mb-4 italic">UNLOCK YOUR POTENTIAL</p>
                    <p className="text-green-400/60 font-bold uppercase tracking-[0.5em] text-sm">North Bangkok University</p>
                </div>
                <div className="flex gap-4">
                    {[...Array(6)].map((_, i) => (
                        <div key={i} className="w-8 h-8 bg-green-400/5 hover:bg-green-400/20 transition-all duration-300 transform -skew-x-12 border border-green-400/10" />
                    ))}
                </div>
            </div>
